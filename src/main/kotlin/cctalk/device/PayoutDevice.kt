...
class PayoutDevice(
    ...
    suspend fun extendedPayoutStatus(): Either<CcTalkError, PayoutStatusExtended> = either {
        ...
        var sensorLevels = getHopperSensors().bind()
        ...
        PayoutStatusExtended(
            ...
            lowLevelSensorStatus = sensorLevels.lowLevel,
            highLevelSensorStatus = sensorLevels.highLevel,
        )
    }
    ...
    private suspend fun getHopperSensors(): Either<CcTalkError, HopperSensorLevels> = either {
        ...
        ensure(levelSensor.dataLength.toInt() >= 1) {
            CcTalkError.DataLengthError(
                1,
                255,
                levelSensor.dataLength.toInt()
            )
        }

        HopperSensorLevels(
            lowLevel = computeSensorStatus(levelSensor.data[0], isUpperLevel = false),
            highLevel = computeSensorStatus(levelSensor.data[0], isUpperLevel = true)
        )
    }

    private fun computeSensorStatus(value: Int, isUpperLevel: Boolean): PayoutSensorStatus {
        // The sensors are reversed somehow
        val supportedFlag = if (isUpperLevel) 0x10 else 0x20
        val isTriggeredFlag = if (isUpperLevel) 0x01 else 0x02

        if ((value.toUByte() and supportedFlag.toUByte()) == 0u.toUByte()) return PayoutSensorStatus.NotSupported
        return if ((value.toUByte() and isTriggeredFlag.toUByte()) == 0.toUByte()) PayoutSensorStatus.Triggered else PayoutSensorStatus.Untriggered
    }
